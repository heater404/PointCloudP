...
public class DD_CoordinateAxis : DD_DrawGraphic
{
    ...
    private void ChangeScale(float ZoomX, float ZoomY)
    {
        ...
        var x = viewSize.x / rangeSize.x;
        var y = viewSize.y / rangeSize.y;
        if (x < 0.01 || y < 0.01)
            return;

        m_CoordinateAxisViewRange.width = x;
        m_CoordinateAxisViewRange.height = y;
    }
        ...
        float markValY, float pixelY, Rect coordinateRect)
    {
        ...
        SetMarkText(markText, new Rect(new Vector2(0, pixelY - (m_MinMarkTextHeight / 2)),
            ...
            RoundToIntStr(markValY, 1000f), true);
    }

    private string RoundToIntStr(float val, float precision)
    {
        return (Mathf.RoundToInt(val * precision) / precision).ToString("f2");
    }
    /// <summary>
